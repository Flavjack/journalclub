...
subtitle: Discover Education
author: Flavio Lozano-Isla; Elizabeth Heros-Aguilar; Andres Casas-Diaz
format:
  html:
    toc: true
    toc-location: left
...
pipeline %>% 
  image_write(format = "eps", "files/Fig1.eps")
  
include_pdf("files/Fig1.pdf")
```

# Import data

```{r}
gsh1 <- "https://docs.google.com/spreadsheets/d/1Wz8-30f1KopqqCrmHSly-rdQkeqH8yR19yuGY76Q-k0/edit#gid=412794809"
# browseURL(gsh)
gs_score <- as_sheets_id(gsh1)

gsh2 <- "https://docs.google.com/spreadsheets/d/1ot6L2RlnsLWSwMhK-SIZUuLLFfHV4cmqRQUe7B7tC_Q/edit#gid=1412841139"
# browseURL(gsh)
...
plot %>% 
  ggsave2(filename = c("files/Fig2.eps")
         , plot = ., width = 20, height = 12, units = "cm")

include_pdf("files/Fig2.pdf")
```
...
pdf("files/Fig3.eps", width = 20/2.5, height = 20/2.5)
...
plot %>% 
  ggsave2(filename = "files/Fig4.eps"
         , plot = ., width = 30, height = 12, units = "cm")

include_pdf("files/Fig4.pdf")
```
